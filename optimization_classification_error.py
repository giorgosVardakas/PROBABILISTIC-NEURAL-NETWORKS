...
class Optimizer:
    ...
    def random_search_gaussian_distribution(self, model_parameters, sigma):
        ...
        print("Iteration=%d, Error=%d" % (iteration, x_best_fvalue))
    ...
    def Nelder_Mead(self, model_parameters):
        ...
        print("Iteration=%d, Error=%d" % (iteration, simplex[0][1]))
...
class Neural_Network:
    ...
    def __forward_pass(self, model_parameters, data):
        ...
        for index, data_i in enumerate(data):
            ...
            if(norm_sum_class_2 < norm_sum_class_1):
                output_vector.append([1, 0])
            else:
                output_vector.append([0, 1])

        return np.asarray(output_vector)

    def error_function(self, model_parameters):
        n_data = self.data.shape[0]
        predicted_classes = self.__forward_pass(model_parameters, self.data)
        classification_error = 0

        for i in range(n_data):
            if (predicted_classes[i] != self.labels[i]).any():
                classification_error += 1
        return classification_error

    def predict(self, model_parameters, data):
        return self.__forward_pass(model_parameters, data)
...
def preprocess_data(data_path):
    ...
    # One-hot encoding for categorical features in labels
